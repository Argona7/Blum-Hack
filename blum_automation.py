...
def get_farming_time(additional_seconds: float, thousandths: int):
    if thousandths:
...
def me(headers: dict):
    ...
    print(Fore.YELLOW + Style.BRIGHT + f"Send me : {response.status_code}")
    time.sleep(1)


def now(headers: dict):
    url = "https://game-domain.blum.codes/api/v1/time/now"
    response = requests.get(url, headers=headers)
    print(Fore.YELLOW + Style.BRIGHT + f"Send now : {response.status_code}")
    time.sleep(1)
...
def my(headers: dict):
    ...
    response = requests.get(url, headers=headers)
    print(Fore.YELLOW + Style.BRIGHT + f"Send my : {response.status_code}")
    time.sleep(1)


def balance(headers: dict):
    url = "https://user-domain.blum.codes/api/v1/friends/balance"
    response = requests.get(url, headers=headers)
    print(Fore.YELLOW + Style.BRIGHT + f"Send balance : {response.status_code}")
    time.sleep(1)


def leaderboard(headers: dict):
    url = "https://tribe-domain.blum.codes/api/v1/tribe/leaderboard"
    response = requests.get(url, headers=headers)
    print(Fore.YELLOW + Style.BRIGHT + f"Send leaderboard : {response.status_code}")
    time.sleep(1)
